...
import { Checkbox, FormControlLabel, Switch, MenuItem, Select, Button } from "@mui/material";
...
export default function Remainders() {
  ...
  const [reminderTime, setReminderTime] = useState({ hour: "08", minute: "00", period: "AM" });
  const [digestTime, setDigestTime] = useState({ hour: "08", minute: "00", period: "AM" });
        ...
        <div className="flex flex-col md:flex-row px-8 py-8 gap-8">
          ...
          <div className="md:w-1/3">
            <p className="text-gray-600 text-sm leading-relaxed">
              You can customize when and how you receive reminders or digests. Choose the days,
              time, and preferred channels (Email, SMS, Slack).
            </p>
          </div>
          ...
          <div className="md:w-2/3 space-y-6">
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={() => setNotifyTaskAssigned(!notifyTaskAssigned)}
                  />
                }
            ...
            <div className="space-y-3">
              <label className="font-medium text-gray-700 text-sm">Send me reminders on</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="font-medium text-gray-700 text-sm block mb-2">Send reminders at</label>
                <div className="flex gap-2">
                  {["hour", "minute", "period"].map((part) => (
                    <Select
                      key={part}
                      value={reminderTime[part]}
                      onChange={(e) => setReminderTime({ ...reminderTime, [part]: e.target.value })}
                      size="small"
                    >
                      {part === "hour"
              ...
              <div>
                <label className="font-medium text-gray-700 text-sm block mb-2">Send reminders to my</label>
            ...
            {/* Digest Section */}
            <div className="space-y-3">
              <label className="font-medium text-gray-700 text-sm">Send me digests on</label>
              <div className="flex flex-wrap gap-2">
                {days.map((d) => (
                  <button
                    key={d}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="font-medium text-gray-700 text-sm block mb-2">Send digests at</label>
                <div className="flex gap-2">
                  {["hour", "minute", "period"].map((part) => (
                    <Select
                      key={part}
                      value={digestTime[part]}
                      onChange={(e) => setDigestTime({ ...digestTime, [part]: e.target.value })}
                      size="small"
                    >
                      {part === "hour"
                        ? Array.from({ length: 12 }, (_, i) => (i + 1).toString().padStart(2, "0")).map((h) => (
              ...
              <div>
                <label className="font-medium text-gray-700 text-sm block mb-2">Send digests to my</label>
                <div className="flex gap-2 flex-wrap">
                  {["Email", "SMS", "Slack"].map((ch) => (
                    <button
                      key={ch}
